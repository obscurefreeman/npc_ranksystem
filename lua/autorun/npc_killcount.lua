...
local idles = {
    ...
    "It's /time/ already, why is /player/ still online?",
    "I've been in /map/ all day, it's /time/ now.",
    "It's /time/, time to clock in.",
    "Is there a rank higher than /rank/ on this server?", 
...
if SERVER then
    ...
    hook.Add("InitPostEntity", "Initaddon", function()
        ...
        local installedGames = 0
        if games then
            for _, game in ipairs(games) do
                if game and game.installed and game.title then
                    table.insert(othergames, game.title)
                    installedGames = installedGames + 1
                end
            end
        end
        
        -- 如果安装的游戏少于3个,添加预设的游戏列表
        if installedGames < 3 then
            othergames = {
                "Half-Life",
                "Half-Life 2",
